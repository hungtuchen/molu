...
import com.people.lyy.client.ParseResponseJSON;
...
public abstract class LKAsyncHttpResponseHandler extends AsyncHttpResponseHandler {
	...
	public void onSuccess(String content) {
		...
		//设置请求完成状态，成功完成1
		this.request.setJobdone(1);
		
//		Object obj = ParseResponseXML.parseXML(request.getMethodTag(), content);
		Object obj = ParseResponseJSON.parseJSON(request.getMethodTag(), content);
		
		Logger.e("success", "try to do success action..." + TransferRequestTag.getRequestTagMap().get(request.getMethodTag()));
	...
	public void onFailure(final Throwable error, final String content) {
		...
		//设置请求完成状态，失败完成2
		this.request.setJobdone(2);
		
		try{
			HttpResponseException exception = (HttpResponseException) error;
			Logger.e("Status Code","" + exception.getStatusCode());
		} catch(Exception e){
			e.printStackTrace();
		}
		
		try{
			Logger.e("error content:", content);
		} catch(Exception e){
			e.printStackTrace();
		}
		
		try{
			Logger.e("failure:", error.getCause().toString());
		} catch(Exception e){
			e.printStackTrace();
		}
		
		try{
			Logger.e("failure message:", error.getCause().getMessage());
		} catch(Exception e){
			e.printStackTrace();
		}
...
//		
//		
//		AlertDialog dialog = new AlertDialog(BaseActivity.getTopActivity());
//		dialog.setTitle("提示");
//		
//		try{
